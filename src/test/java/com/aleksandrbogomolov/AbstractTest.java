...
public abstract class AbstractTest {
    ...
    private final String loginUrl = "http://localhost:8888/users/login";

    private int startId = 1;

    protected final User loggedUser = new User("" + startId, "User", "user@mail.ru", "password", 2000, true, LocalDate.of(2016, 11, 7), new HashSet<>(Arrays.asList(Role.ROLE_ADMIN, Role.ROLE_USER)));
    ...
    protected Response getResponseForCSRF() {
        return given().auth().basic(loggedUser.getName(), loggedUser.getPassword())
                      .when().post(loginUrl)
